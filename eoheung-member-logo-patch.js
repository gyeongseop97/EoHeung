(function(){
  ...
  function currentMember(){
    ...
    const user=state.user||{},uid=String(user.id||''),email=String(user.email||'').trim().toLowerCase();
    return (uid?(state.members||[]).find(m=>String(m.auth_user_id||'')===uid):null)||(email?(state.members||[]).find(m=>String(m.email||'').trim().toLowerCase()===email):null)||null;
  }
  function canEditAbout(){return currentMember()?.member_role==='admin'}
...
function plannedWatchGames(){
  ...
  function ensureHistoryPanel(){const settings=$('#settings');if(!settings)return;const host=$('#settingsHistorySlot')||settings;let card=$('#eoHistoryPanel');if(!card){card=document.createElement('div');card.id='eoHistoryPanel';card.className='card pad';card.style.marginTop='14px';card.innerHTML='<div class="eo-dashboard-card-title"><h3>변경 이력</h3><div class="toolbar" style="margin:0"><button class="btn secondary" data-refresh-history>새로고침</button></div></div><p class="note">회원 수정, 직관 체크, 좌석/메모 저장 등 주요 변경 내역을 Supabase 서버에 기록합니다.</p><div id="eoHistoryList" class="eo-history-list"></div>'}if(card.parentElement!==host)host.appendChild(card)}
  ...
  function bindOnce(){if(window.__eoheungFinalFixBound)return;window.__eoheungFinalFixBound=true;document.body.addEventListener('click',async e=>{const nav=e.target.closest('[data-eo-next-nav]');if(nav){e.preventDefault();const games=plannedWatchGames();const dir=Number(nav.dataset.eoNextNav||0);window.__eoNextWatchIndex=Math.max(0,Math.min(games.length-1,Number(window.__eoNextWatchIndex||0)+dir));renderNextWatchPanel();return}const seat=e.target.closest('[data-save-seat]');if(seat){e.preventDefault();await saveGameSeat(seat.dataset.saveSeat);return}if(e.target.closest('[data-save-about]')){e.preventDefault();await saveAbout();return}if(e.target.closest('[data-reset-about]')){e.preventDefault();if(!canEditAbout()){syncAboutAdminAccess();toast?.('관리자만 모임 소개와 운영 규칙을 수정할 수 있습니다.');return}if(confirm('모임 소개와 운영 규칙을 기본값으로 되돌릴까요?')){localStorage.removeItem(LS_ABOUT);await ensureAboutPage(true)}return}if(e.target.closest('[data-refresh-history]')){e.preventDefault();await renderHistoryPanel();toast?.('서버 변경이력을 새로고침했습니다.');return}},true);document.body.addEventListener('change',e=>{if(e.target.matches('input[type="checkbox"][data-game]'))setTimeout(()=>recordChange('직관 체크 변경',`경기 ID ${e.target.dataset.game} / 회원 ID ${e.target.dataset.member} / ${e.target.checked?'체크':'해제'}`,'attendance',e.target.dataset.game),120)})}
