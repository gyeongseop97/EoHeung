(function(){
  ...
  function isMobileLike(){
    return window.innerWidth <= 900 || window.matchMedia('(hover:none) and (pointer:coarse) and (max-width:920px)').matches;
  }
  ...
  function injectLayoutFixStyle(){
    if(byId('eoheungLayoutFixStyle')) return;
...
@media(max-width:900px){
  ...
  function ensureMobileDrawer(){
    ...
    if(!byId('mobileDrawerFooter')){
      ...
      footer.querySelector('.mobile-logout-btn').addEventListener('click', e => {
  ...
  function enhanceScheduleControls(){
    const manual = byId('openGameModalBtn');
    const sync = byId('callSyncBtn');
    if(manual) manual.classList.add('schedule-hidden-action');
    if(sync) sync.classList.add('schedule-hidden-action');
    const head = $('#schedule .calendar-head');
    const prev = byId('prevMonthBtn');
    const today = byId('todayBtn');
    const next = byId('nextMonthBtn');
    if(!head || !prev || !today || !next) return;
    let controls = byId('calendarNavControls');
    if(!controls){
    ...
    controls.appendChild(prev); controls.appendChild(today); controls.appendChild(next);
  }

  async function syncTicketlinkServerTime(force=false){
    if(clockState.syncing && !force) return;
    clockState.syncing = true;
    clockState.source = clockState.lastSync ? 'resyncing' : 'syncing';
    updateLinkClock();
    const started = Date.now();
    const url = TICKETLINK_HOST + '?_eoheung_time=' + started;
    try{
      let res;
      try{
        res = await fetch(url, { method:'HEAD', cache:'no-store', mode:'cors', credentials:'omit' });
      }catch(headError){
        res = await fetch(url, { method:'GET', cache:'no-store', mode:'cors', credentials:'omit' });
      }
      const ended = Date.now();
      const dateHeader = res.headers.get('date');
      if(!dateHeader) throw new Error('Date header 없음');
      const serverMs = Date.parse(dateHeader);
      if(!Number.isFinite(serverMs)) throw new Error('Date header 파싱 실패');
      const rtt = ended - started;
      clockState.offsetMs = serverMs + Math.round(rtt / 2) - ended;
      clockState.rtt = rtt;
      clockState.lastSync = new Date();
      clockState.source = 'server';
      clockState.error = '';
    }catch(err){
      clockState.source = 'fallback';
      clockState.error = err && err.message ? err.message : String(err);
    }finally{
      clockState.syncing = false;
      updateLinkClock();
    }
  }

  function clockNow(){
    return clockState.source === 'server' ? new Date(Date.now() + clockState.offsetMs) : new Date();
  }

  function linkClockCardHtml(){
    const status = clockState.source === 'server' ? '서버 동기화' : clockState.source === 'fallback' ? '직접 동기화 실패' : '동기화 중';
    const sub = clockState.source === 'server'
      ? `Ticketlink Date 헤더 기준 · RTT ${clockState.rtt ?? '-'}ms`
      : clockState.source === 'fallback'
        ? `브라우저 차단 시 KST 표시 · ${safeEsc(clockState.error || '')}`
        : '티켓링크 서버 Date 헤더 확인 중';
    return '<div id="linkClockCard" class="card pad quick-link-card link-clock-card">'
      + '<div><div class="link-clock-title">티켓링크 서버시간 <span class="sync-state">'+safeEsc(status)+'</span></div>'
      + '<div id="linkClockSub" class="link-clock-sub '+(clockState.source==='fallback'?'link-clock-error':'')+'">'+sub+'</div></div>'
      + '<div id="linkClockTime" class="link-clock-time">--:--:<span class="link-clock-sec">--.---</span></div>'
      + '<div class="link-clock-actions"><a class="primary" href="'+TICKETLINK_URL+'" target="_blank" rel="noopener">티켓링크 열기</a><button type="button" id="linkClockSyncBtn">재동기화</button></div>'
      + '</div>';
  }

  function updateLinkClock(){
    const el = byId('linkClockTime');
    if(!el) return;
    const now = clockNow();
    ...
    const sub = byId('linkClockSub');
    if(sub){
      sub.classList.toggle('link-clock-error', clockState.source === 'fallback');
      sub.textContent = clockState.source === 'server'
        ? `Ticketlink Date 헤더 기준 · RTT ${clockState.rtt ?? '-'}ms`
        : clockState.source === 'fallback'
          ? `직접 동기화 실패 시 KST 표시 · ${clockState.error || ''}`
          : '티켓링크 서버 Date 헤더 확인 중';
    }
    const badge = $('.link-clock-title .sync-state');
    if(badge) badge.textContent = clockState.source === 'server' ? '서버 동기화' : clockState.source === 'fallback' ? 'KST 임시표시' : '동기화 중';
  }

  function installQuickLinkFeatures(){
    if(window.__quickLinkFeaturesInstalled) return;
    window.__quickLinkFeaturesInstalled = true;

    function enhancedRenderLinks(){
      const root = byId('linkList');
      if(!root) return;
      ...
      root.innerHTML = linkClockCardHtml() + links.map(l => {
    ...
    document.body.addEventListener('click', async e => {
      const syncBtn = e.target.closest('#linkClockSyncBtn');
      if(syncBtn){ e.preventDefault(); syncTicketlinkServerTime(true); return; }
      ...
      try{
        ...
        state.links = state.links.filter(x => String(x.id) !== String(id));
        enhancedRenderLinks();
  ...
  function syncMobileUser(){
    const target = $('#mobileDrawerFooter .mobile-drawer-user');
    const source = byId('userEmail');
  ...
  function init(){
    ...
    window.addEventListener('resize', () => { runFixes(); if(!isMobileLike()) closeMenu(); });
